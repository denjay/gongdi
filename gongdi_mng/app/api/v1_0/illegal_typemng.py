...
def illegal_types_post(body,jwt = None):
    try:
...
def illegal_types_id_get(id,jwt = None):
...
def illegal_types_id_put(id,body,jwt = None):
    try:
...
def illegal_types_id_delete(id,jwt = None):
    try:
...
@auth.valid_login
@p.check("illegal_type",["view"])
def illegal_categorys_id_types_get(id,jwt = None):
