########################################
# create by :cxh-PC
# create time :2018-03-02 15:55:36.875530
########################################
...
def guifang_docs_get(page = None,per_page = None,buwei = None,doc_name = None,jwt = None) -> str:
    return docs_get(Guifang_doc,page,per_page,buwei,doc_name)
@auth.valid_login
@p.check("guifang_doc",["insert"])
def guifang_docs_post(body,jwt = None) -> str:
    return docs_post(Guifang_doc,body)
@auth.valid_login
@p.check("guifang_doc",["view"])
def guifang_docs_id_get(id,jwt = None) -> str:
    return docs_id_get(Guifang_doc,id)
@auth.valid_login
@p.check("guifang_doc",["edit"])
def guifang_docs_id_put(id,body,jwt = None) -> str:
    return docs_id_put(Guifang_doc,id,body)
@auth.valid_login
@p.check("guifang_doc",["delete"])
def guifang_docs_id_delete(id,jwt = None) -> str:
