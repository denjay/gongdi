########################################
# create by :cxh-PC
# create time :2018-03-02 15:55:36.665340
########################################
...
def buweis_post(body,jwt = None) -> str:
    try:
        data = Buwei(**body)
        db.session.add(data)
        db.session.commit()
...
def buweis_id_delete(id,jwt = None) -> str:
    try:
...
@auth.valid_login
@p.check("buwei",["view"])
def dantis_id_buweis_get(id,page = None,per_page = None,jwt = None) -> str:
...
@auth.valid_login
def bw_jishu_qrcode_id_get(id,jwt = None) -> str:
    try:
        ...
        return {"error": str(e)}, 422, {"content-type": "chatset=utf8"}
@auth.valid_login
def bw_quality_qrcode_id_get(id,jwt = None) -> str:
    try:
        ...
        return {"error": str(e)}, 422, {"content-type": "chatset=utf8"}
@auth.valid_login
def bw_safety_qrcode_id_get(id,jwt = None) -> str:
    try:
        ...
        return {"error": str(e)}, 422, {"content-type": "chatset=utf8"}
@auth.valid_login
def bw_produce_qrcode_id_get(id,jwt = None) -> str:
    try:
